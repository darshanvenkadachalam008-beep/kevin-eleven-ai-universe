    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 md:px-6 py-3 bg-background/60 backdrop-blur-xl border-b border-border/50">
        <Link to="/" className="flex items-center gap-2">
          <span className="font-display text-xl md:text-2xl font-bold text-primary neon-text tracking-widest">KEVIN</span>
          <span className="font-display text-xs md:text-sm font-medium text-secondary tracking-wider opacity-70">ELEVEN</span>
        </Link>
            ...
            {navItems.map(item => (
              <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)}
                className={`w-full text-center px-4 py-3 rounded-lg font-display text-sm tracking-wider uppercase ${
                  location.pathname === item.path
                    ? 'text-primary bg-primary/10 border border-primary/30' : 'text-muted-foreground'
                }`}>
                {item.label}
              </Link>
            ))}
